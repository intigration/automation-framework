...
#
# DESCRIPTION :
# Verify for ext4 filesystem journal disable support
#
...
echo "About to run ext4 filesystem journal disable test..."
...
#Check the disabling of ext4 journal
journal_check() {
	#Creating filesystem image
	echo "Creating an 100MB image"
	dd if=/dev/zero of=$IMAGE_PATH bs=1M count=100
	exit_on_step_fail "ext4_journal_disable_image_create"
	...
	#Enabling writeback  mode of the ext4 journal
	echo "Enabling the writeback  mode for EXT4 journal"
	tune2fs -o journal_data_writeback $IMAGE_PATH
	exit_on_step_fail "ext4_journal_disable_writeback_mode_set"

	#Disabling the journal
	echo "Disabling the EXT4 journal"
	tune2fs -O ^has_journal $IMAGE_PATH
	lava_test_result "ext4_journal_disable"

	#Examine the errors of EXT4 filesystem
	echo "Check any errors for EXT4 filesystem"
	e2fsck -p  $IMAGE_PATH
	lava_test_result "ext4_journal_disable_error_check"
	...
	lava_test_result "ext4_journal_disable_mount_check"

	#Check for the journal flag
	echo "check for journal flag"
	dumpe2fs $IMAGE_PATH | grep journal
	lava_test_result "ext4_journal_disable_flag_check"

	#unmount the filesystem image
	echo "Unmount the filesystem image"
	umount $MOUNT_PATH
	lava_test_result "ext4_journal_disable_unmount_check"
}
